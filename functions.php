...
{
    $productslist = readFromFile(PRODUCTS_FILE);

    return $productslist;
}
function adminUpdateProductsEndpoint()
{
    $name = $_POST['name'] ?? '';
    $price = $_POST['price'] ?? 0;
    $id = $_POST['id'] ?? 0;
    ...
    if (strlen($name) < 3) {
        header("Location: /?action=adminProducts&error=Product name should be at least 3 symbols length!");
        return;
    }
    if ($id === 0) {
        header("Location: /?action=adminProducts&error=Undefined product to update. Please send us ID");
        return;
    }

    if (!is_numeric($price)) {
        header("Location: /?action=adminProducts&error=Price should be numeric");
        return;
    }
    ...
    if (!$updated) {
        header("Location: /?action=adminProducts&error=The product with id $id does not exists");
        return;
    }

    writeFile(PRODUCTS_FILE, $products);

    header("Location: /?action=adminProducts&message=The product with id=$id has been updated!");

}
function adminRemoveProductEndpoint()
{
    $productId = $_GET['productId'] ?? '';

    if (strlen($productId) != 32) {
        header("Location: /?action=adminProducts&error=Wrong ID given");
    }
    ...
    if (!$deleted) {
        header("Location: /?action=adminProducts&error=404 not found! The product you want to remove does not exist!");
    }

    writeFile(PRODUCTS_FILE, array_values($products));

    header("Location: /?action=adminProducts&message=The product with id=$productId has been removed!");
}
function adminRemoveUser()
{
    ...
    if (strlen($userid) != 32) {
        header("Location: /?action=adminUsers&error=Wrong ID given");
    }
    ...
    if (!$deleted) {
        header("Location: /?action=adminUsers&error=404 not found! The user you want to remove does not exist!");
    }

    writeFile(USERS_FILE, array_values($users));

    header("Location: /?action=adminUsers&message=The user with id=$userid has been removed!");
}
